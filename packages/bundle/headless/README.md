...
- **One submitted task only** — the runner has no interactive follow-up surface; it waits through Agent work and active supervised workflows triggered by that task, then prints the last non-empty assistant message in the complete interval. A paused, needs-input, or budget-limited workflow is a quiescent parked state and is interrupted when the one-shot Agent exits rather than holding the process open.
