...
The current Client assembly explicitly mounts each selected business Remote contribution, including the read-only Host plugin inventory (`pluginInventory/list`), session-cwd-fenced saved-workflow summaries (`workflowDefinitions/list`), and the bounded workflow-run roster and detail methods (`workflowRuns/*`). Cordis effect ownership withdraws every contribution when this assembly unloads, while `@deepseek-ai/dsh-api-gateway/client` owns descriptor validation, traced namespace Services, direct and scoped methods, invocation, and cancellation. The Client entry consumes the shared `TypertClientRemote` interface through Cordis and does not import the concrete Gateway. It re-exports the Gateway Client face's declaration merges type-only, so a consumer reaching the forwarded-event types through this facade gains no runtime edge to the Gateway implementation.
...
`workflows/run-change` carries one revisioned bounded run-head change or an invalidation. Complete member rosters, outcomes, logs, results, and artifacts stay behind the paged `workflowRuns/*` methods and never enter the forwarded event.
