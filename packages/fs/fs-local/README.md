...
The **local-filesystem implementation** of the `ctx.fs` provider contract ([`@deepseek-ai/dsh-fs`](../fs)). Backs the fourteen `FileSystem` primitives with the host filesystem; loading it as a plugin populates `ctx.fs`.
...
- **`readBytesNoFollow`** — on POSIX, opens the lexical path with `O_NOFOLLOW`, validates the opened descriptor as a regular file, and reads at most one byte beyond the required cap through that descriptor. A final symbolic link is `FS_NOT_REGULAR_FILE`; ancestor links retain ordinary resolution. The pathname can be replaced after open without changing the object read.
...
- **`writeTextNoFollow`** — on POSIX, stages and fsyncs text beside the lexical destination, rejects a final link or non-file, and applies the required `createIfAbsent`/`replaceIfVersion` guard again at publication. Creation uses the no-replace hard link; replacement rechecks the final entry with `lstat` immediately before atomic rename, so a substituted link is rejected without opening or changing its target. It deliberately returns no prior-content diff basis.
...
- **Final-component no-follow operations are POSIX-only** — the local provider reports `FS_IO_ERROR` on Windows rather than claiming equivalent descriptor/publication guarantees without native handle-based implementations.
