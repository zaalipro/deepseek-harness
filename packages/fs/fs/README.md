...
A backend subclasses `FileSystem` and implements fourteen primitives. Providers that cannot guarantee either final-component no-follow operation inherit a loud `FS_IO_ERROR`; they must not emulate it with separate metadata and I/O calls.
...
| `readBytesNoFollow(path, opts, signal, maxBytes)` | Open the final path component without following a symbolic link, validate it as a regular file, and perform the bounded read through that same descriptor or provider object. Ancestor links retain the backend's normal rules. |
...
| `writeTextNoFollow(path, opts, content, expected, signal?)` | Guarded path-shaped publication that never follows the final symbolic-link component. The provider applies the required create/version guard and final-entry validation inside the same operation at publication. |
...
- **Text-only mutations by contract** — text reads and mutations use text semantics; `readBytes` and `readBytesNoFollow` are the raw-byte primitives, while binary-safe mutations remain a deliberate deferral of [the tool-schemas Agent Note](../../../.agents/notes/implemented/feature/2026-06-17-filesystem-tool-schemas.md).
- **Fourteen primitives only** — no delete, rename/move, copy, or watch; `listDir` is single-level, with recursion, globbing, pagination, and search out of scope per [the directory-listing Agent Note](../../../.agents/notes/archived/architecture/2026-07-03-filesystem-directory-listing-seam.md).
