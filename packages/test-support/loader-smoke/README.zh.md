...
`runFixtureTurn` 通过恰好一个已配置的根 agent（智能体）驱动一项任务，在该任务进入持久收件箱后转发规范事件，刷写会话，并返回最终 assistant 文本和累计用量。可选的 `drain(agent)` 回调会为应用特有的后台工作保持观测区间开启；调用方负责等待这些工作完全停稳。示例本地 driver 继续负责配置、渲染和断言。
