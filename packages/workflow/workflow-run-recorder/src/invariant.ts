/** Package-owned durable workflow-record invariants. @module @deepseek-ai/dsh-workflow-run-recorder/invariant */
...
const PACKAGE_NAME = '@deepseek-ai/dsh-workflow-run-recorder'

/** Cordis companion plugin name. */
export const name = 'workflow-run-recorder-invariant'
...
function applyEvent(trace: WorkflowTrace, event: SessionEvent, fail: InvariantFailure): void {
  ...
  switch (event.type) {
    ...
    case 'tool-workflow/run-end': {
      const run = openRun(trace, runId, event.type, fail)
      if (data.stopReason !== 'completed' && data.stopReason !== 'cancelled'
        && data.stopReason !== 'error' && data.stopReason !== 'interrupted') {
