...
- **不支持原子的最终组件不跟随操作**：`readBytesNoFollow` 与 `writeTextNoFollow` 继承 Service Definition 明确的 `FS_IO_ERROR`。提供方绝不会把 E2B 元数据和 SDK 读写调用组合成虚假的原子保证；因此已保存工作流的文件读取与保存需要另一个文件系统提供方。
...
- **不支持最终组件不跟随读取与写入**：实现它们需要由单个远程进程或提供方对象在验证／读取期间持有打开文件，并提供带防护的路径条目发布。该 POC 会明确失败，而不是削弱任一保证。
