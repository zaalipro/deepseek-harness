...
- **No atomic final-component no-follow operations** — `readBytesNoFollow` and `writeTextNoFollow` inherit the Service Definition's loud `FS_IO_ERROR`. The provider never composes E2B metadata and SDK read/write calls into a false atomic guarantee; saved-workflow file reads and saves therefore require another filesystem provider.
...
- **Final-component no-follow reads and writes are unsupported** — implementing them requires one remote process or provider object to hold the opened file through validation/read, plus guarded path-entry publication. The POC fails loud instead of weakening either guarantee.
