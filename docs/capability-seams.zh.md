...
| `ctx.workflowEngine` | `seam` | [`workflow`](../packages/workflow/workflow) | [`workflow-worker-thread`](../packages/workflow/workflow-worker-thread) | [`tool-ralph`](../packages/workflow/tool-ralph), [`workflow-supervisor`](../packages/workflow/workflow-supervisor) | - | 每个上下文一个引擎，与 bash 类似，不设具名提供方注册表；固定的 Ralph 消费方和逻辑运行监督器会启动尝试，其中 `agent()` 调用通过 `ctx.subagents` 扇出。 |
...
| `ctx.workflowSupervisor` | `core` | [`workflow-supervisor`](../packages/workflow/workflow-supervisor) | - | [`command-workflows`](../packages/workflow/command-workflows), [`tool-workflow`](../packages/workflow/tool-workflow), [`workflow-run-recorder`](../packages/workflow/workflow-run-recorder) | - | 在活动 `WorkflowRun` 尝试之上负责稳定逻辑运行：后台启动、journal 暂停／恢复、有界保留 manifest、类型化 Remote 读取与控制，以及精确所有者完成交付。 |
| `ctx.workflowRunRecorder` | `core` | [`workflow-run-recorder`](../packages/workflow/workflow-run-recorder) | - | [`command-workflows`](../packages/workflow/command-workflows), [`tool-workflow`](../packages/workflow/tool-workflow) | - | 把一次显式的顶层监督器启动归属到其父 Session，并追加持久 Chat 节点使用的逻辑运行与成员生命周期；嵌套和无法归属的启动只出现在仪表板中。 |
