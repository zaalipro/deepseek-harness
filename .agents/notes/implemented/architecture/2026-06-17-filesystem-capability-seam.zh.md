...
- 打开最终路径组件而不跟随符号链接，然后通过同一个描述符或提供方对象完成验证与有界原始字节读取。
- 创建或替换一个 UTF-8 文本文件。
- 发布带防护的 UTF-8 文本而不跟随最终路径组件，把必填的创建／版本防护和最终条目验证保留在同一个提供方操作内。
- 通过字面替换编辑一个已有的 UTF-8 文本文件。

两个路径形态的不跟随操作供把来源或目标路径名本身视为权威的消费方使用。它们关闭组合 `lstat`、`resolve` 与目标形态 I/O 所产生的替换窗口。无法提供描述符或发布保证的提供方以 `FS_IO_ERROR` 失败，绝不通过分离检查模拟任一操作。本地提供方在 POSIX 上实现两者，在 Windows 原生句柄支持完成前明确失败；E2B 概念验证也明确失败。祖先链接包含关系仍是所属提供方或沙箱策略的独立责任。
...
- **最终组件替换。** 确定性钩子会在不跟随读取描述符打开后，以及带防护写入发布前立即把路径名替换为符号链接。读取返回已打开文件的内容；写入拒绝被替换的条目，并保持其链接目标不变。
