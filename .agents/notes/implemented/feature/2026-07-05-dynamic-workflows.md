...
A workflow call contains JSON `meta` (`name`, `description`, and optional `whenToUse`/`phases`) and a JavaScript `script` body with top-level `await` that completes with a JSON value. Metadata is validated as data and never evaluated. The body receives `agent(prompt, options)`, `parallel(thunksOrJobMaps)`, `pipeline(items, ...stages)`, `phase(title)`, `log(message)`, `complete(value)`, `pause()`/`await_user()`, `budget()`, scratch-file helpers, and `args`. A declarative job-map panel validates every job and reserves all live agent calls atomically before launching a branch; a thunk panel admits only the concrete `agent()` calls reached during execution because a thunk may contain zero, many, or nested calls. Pipeline stages receive `(prev, item, index)` with no cross-stage barrier; failed children and ordinary stage errors resolve the affected item to `null` and skip its remaining stages. Authored control flow derives from `args` and committed host results: clock, randomness, shared-memory atomics, weak references, and finalizers are unavailable so the generalized host-call journal reaches the same identities. That journal retains child and scratch-read results, observer and scratch-write effects, and satisfied human gates; replay returns results, suppresses repeated effects and gates, restores phase state, and rejects divergence.
...
The `workflow` tool resolves one saved name, inline script, or file source, then asks `ctx.workflowSupervisor` to publish a logical run. A successful ordinary call returns the display handle immediately and leaves execution, cancellation, resume, retention, and completion delivery with the supervisor; `validate_only` is the sole collected smoke-check path. Render intent is a `generic` card whose title derives from `name`, then `meta.name`, then the fixed fallback, so presentation remains a pure function of args. The tool description is the model-facing authoring spec. The usage policy ships with the tool as its own `tool:<toolName>` prompt section (explicit-ask-only guidance — tool guidance lives in tool plugins, never in the deployment persona); the harness has no ultracode-style effort gate.

`dsh-workflow-run-recorder` projects an explicitly attributed top-level launch into the calling parent Session as four log-only `tool-workflow/*` event kinds. Tool and command consumers each wrap exactly one supervisor launch; nested, internal, and unattributed launches remain dashboard-only. The stable supervised run id spans engine attempts, and only one terminal logical event closes the record; launching Turn closure is not interruption. Recording observes rather than controls execution: its first append failure disables later writes for that run and leaves a legal prefix without changing execution. [`ui-workflow-run`](../../../../packages/client/ui-workflow-run/README.md) rebuilds those facts through the Conversation Node engine as a separate keyed Chat row; the existing generic tool row remains its own presentation owner. The detailed persistence, replay, disclosure, and child-navigation decision lives in [durable workflow runs in Chat](2026-08-10-durable-workflow-runs-in-chat.md).
...
- **Cross-process resume** — a same-process pause retains a host-call journal and resumes the original script/args/budget; process death marks active runs `Interrupted` rather than resumable. See [saved workflows and the run supervisor](2026-08-17-saved-workflow-supervisor.md).
- **Nested `workflow()`** (Grok also forbids launching workflows from workflows; inline instead) and the `effort`/`isolation`/`agentType` agent options (each rejects loud with a message naming it deferred).
- **An overall run wall-clock timeout** — cancellation always frees the caller, so a cap on total run time is a policy knob, not a correctness need.
...
Fan-out plans live in rerunnable scripts, and `outputSchema` provides authoritative structured child results. Each engine attempt pays worker startup and message-port RPC costs, but host startup stays non-blocking, cancellation can terminate the worker, and serialization enforces the value boundary. Worker threads are not a security boundary. Invalid options fail rather than degrading to Claude Code's `null`; the supervisor retains control through each run handle while observers receive snapshots only. The ordinary tool publishes background logical runs, and top-level Web users receive a durable, replayable record without widening the execution seam or coupling the original tool card to workflow-specific UI.
